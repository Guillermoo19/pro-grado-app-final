        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h3 class="text-2xl font-bold mb-6 text-chamos-marron-oscuro">{{ __('Historial de Mis Pedidos') }}</h3>
                                    ...
                                    @foreach ($pedidos as $pedido)
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                {{-- ESTA RUTA ES PARA EL CLIENTE NORMAL --}}
                                                <a href="{{ route('pedidos.show', $pedido->id) }}" class="inline-flex items-center px-3 py-1 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 focus:bg-indigo-700 active:bg-indigo-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">Ver Detalles</a>
                                                @if ($pedido->estado_pago === 'pendiente' && $pedido->estado_pedido !== 'cancelado')
                                                    <a href="{{ route('checkout.confirm', $pedido->id) }}" class="inline-flex items-center px-3 py-1 bg-green-500 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-600 focus:bg-green-600 active:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition ease-in-out duration-150 ml-2">Subir Comprobante</a>
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
