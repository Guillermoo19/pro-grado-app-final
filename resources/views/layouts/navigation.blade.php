<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                    ...
                    {{-- Enlace a Mis Pedidos (para clientes) --}}
                    @auth
                        <x-nav-link :href="route('pedidos.index')" :active="request()->routeIs('pedidos.index')">
                            {{ __('Mis Pedidos') }}
                        </x-nav-link>
                    @endauth
                    {{-- Enlace al Catálogo de Productos (público) --}}
                    <x-nav-link :href="route('productos.index')" :active="request()->routeIs('productos.index')">
                        {{ __('Catálogo') }}
                    </x-nav-link>
                    {{-- Enlace al Carrito (público) --}}
                    <x-nav-link :href="route('carrito.index')" :active="request()->routeIs('carrito.index')">
                        {{ __('Carrito') }}
                    </x-nav-link>

                    {{-- Enlaces de administración (solo si el usuario es admin) --}}
                    @auth
                        @if (Auth::user()->isAdmin())
                            <x-nav-link :href="route('pedidos.admin_index')" :active="request()->routeIs('pedidos.admin_index')">
                            ...
                            <x-nav-link :href="route('ingredientes.index')" :active="request()->routeIs('ingredientes.index')">
                                {{ __('Ingredientes') }}
                            </x-nav-link>
                        @endif
                    @endauth
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @auth
                @if (Auth::user()->isAdmin())
                    <x-responsive-nav-link :href="route('pedidos.admin_index')" :active="request()->routeIs('pedidos.admin_index')">
                        {{ __('Gestión de Pedidos') }}
                    </x-responsive-nav-link>
                    ...
                    <x-responsive-nav-link :href="route('ingredientes.index')" :active="request()->routeIs('ingredientes.index')">
                        {{ __('Ingredientes') }}
                    </x-responsive-nav-link>
                @endif
            @endauth
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            @auth {{-- Solo muestra las opciones si el usuario está autenticado --}}
                <div class="px-4">
