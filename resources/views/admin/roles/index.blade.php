                ...
                <div class="p-6 text-gray-900">
                    <div class="flex justify-between items-center mb-4">
                        <div class="flex items-center space-x-4">
                            ...
                            <h3 class="text-2xl font-bold text-chamos-marron-oscuro">{{ __('Lista de Roles') }}</h3>
                        </div>
                        <a href="{{ route('admin.roles.create') }}" class="inline-flex items-center px-4 py-2 bg-yellow-400 border border-transparent rounded-md font-semibold text-xs text-black uppercase tracking-widest hover:bg-yellow-500 active:bg-yellow-600 focus:outline-none focus:border-yellow-600 focus:ring ring-yellow-300 disabled:opacity-25 transition ease-in-out duration-150">
                            {{ __('Crear Nuevo Rol') }}
                        </a>
                    </div>
                    ...
                    @if ($roles->isEmpty())
                        <p class="text-gray-600">{{ __('No hay roles registrados en el sistema.') }}</p>
                    @else
                                    ...
                                    @foreach ($roles as $role)
                                        <tr class="bg-white border-b hover:bg-gray-50">
                                            ...
                                            <td class="py-4 px-6 flex items-center space-x-2">
                                                <a href="{{ route('admin.roles.edit', $role->id) }}" class="inline-flex items-center px-4 py-2 bg-blue-200 border border-transparent rounded-md font-semibold text-xs text-black uppercase tracking-widest hover:bg-blue-300 focus:outline-none focus:border-blue-400 focus:ring ring-blue-100 disabled:opacity-25 transition ease-in-out duration-150">
                                                    {{ __('Editar') }}
                                                </a>
                                                <form action="{{ route('admin.roles.destroy', $role->id) }}" method="POST" onsubmit="return confirm('¿Estás seguro de que quieres eliminar este rol? Esta acción es irreversible.');">
                                                    @csrf
                                                    @method('DELETE')
