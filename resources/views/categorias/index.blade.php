            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <a href="{{ route('admin.categorias.create') }}" class="inline-flex items-center px-4 py-2 bg-chamos-amarillo border border-transparent rounded-md font-semibold text-xs text-chamos-marron-oscuro uppercase tracking-widest hover:bg-yellow-400 focus:bg-yellow-400 active:bg-yellow-500 focus:outline-none focus:ring-2 focus:ring-chamos-amarillo focus:ring-offset-2 transition ease-in-out duration-150"> {{-- CORREGIDO: admin.categorias.create --}}
                                ...
                                @forelse ($categorias as $categoria)
                                    <tr class="hover:bg-gray-100">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            ...
                                            <a href="{{ route('admin.categorias.edit', $categoria->id) }}" class="inline-flex items-center px-3 py-1 bg-blue-200 border border-transparent rounded-md font-semibold text-xs text-black uppercase tracking-widest hover:bg-blue-300 focus:bg-blue-300 active:bg-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-200 focus:ring-offset-2 transition ease-in-out duration-150 mr-2">Editar</a> {{-- CORREGIDO: admin.categorias.edit --}}
                                            {{-- Botón Eliminar: Rojo con texto blanco --}}
                                            <form action="{{ route('admin.categorias.destroy', $categoria->id) }}" method="POST" class="inline-block" onsubmit="return confirm('¿Estás seguro de que quieres eliminar esta categoría?')"> {{-- CORREGIDO: admin.categorias.destroy --}}
                                                @csrf
                                                @method('DELETE')
