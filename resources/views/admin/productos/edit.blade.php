            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900">
                        <h3 class="text-2xl font-bold mb-6 text-chamos-marron-oscuro">{{ __('Formulario de Edición de Producto') }}</h3>
                        ...
                        <form method="POST" action="{{ route('admin.productos.update', $producto->id) }}" enctype="multipart/form-data" class="space-y-6">
                            ...
                            <div>
                                ...
                                <input id="nombre" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" type="text" name="nombre" value="{{ old('nombre', $producto->nombre) }}" required autofocus />
                                @error('nombre')
                            ...
                            <div>
                                ...
                                <textarea id="descripcion" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" name="descripcion">{{ old('descripcion', $producto->descripcion) }}</textarea>
                                @error('descripcion')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input id="precio" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" type="number" step="0.01" min="0" name="precio" value="{{ old('precio', $producto->precio) }}" required />
                                @error('precio')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input id="stock" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" type="number" min="0" name="stock" value="{{ old('stock', $producto->stock) }}" required />
                                @error('stock')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                <label for="imagen" class="block font-medium text-sm text-gray-700">{{ __('Imagen del Producto (Opcional)') }}</label>
                                ...
                                @if ($producto->imagen)
                                    <div class="mt-2">
                                        <p class="text-sm text-gray-600">{{ __('Imagen actual:') }}</p>
                                        <img src="{{ asset('storage/' . $producto->imagen) }}" alt="{{ $producto->nombre }}" class="w-24 h-24 object-cover rounded-md mt-1">
                                    </div>
                                @endif
                            </div>
                            ...
                            <div>
                                ...
                                <select id="ingredientes" name="ingredientes[]" multiple class="block mt-1 w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 h-48">
                                    @foreach ($ingredientes as $ingrediente)
                                        <option value="{{ $ingrediente->id }}" {{ in_array($ingrediente->id, old('ingredientes', $producto->ingredientes->pluck('id')->toArray())) ? 'selected' : '' }}>
                                            {{ $ingrediente->nombre }}
                                        </option>
                                    @endforeach
                                </select>
